...
public class ProductoServiceimpl implements IProductosService {
    ...
    public ProductoResponse getAllProductoByProveedor(int numPage, int sizePage, String orderBy, String sortDir, String nombreProveedor) {
        ...
        log.info("SE EMPEZARA A BUSCAR EL MEDIANTE LA FORMA, {}", sortDir);
        //Buscamos y mapeamos a nuestro DTO
        Page<ProductosModel> productos = iProducto.findByProveedor_NombreProveedor(nombreProveedor,pageable);
        List<ProductosModel> listaProductos = productos.getContent();
        List<ProductosDTO> contenido = listaProductos
                .stream()
                .map(producto -> mapearDTOEntidad(producto))
    ...
    public ProductosDTO saveProducto(ProductosDTO producto) {
        ProductosModel productoNuevo = iProducto.save(mapearEntidadDTO(producto));
        return mapearDTOEntidad(productoNuevo);
    }

    @Override
    public void updateProductos(ProductosDTO producto) {
        ProductosModel productoBuscado = iProducto.findById(producto.getIdProducto()).orElseThrow(()-> new ResourceNotFoundException("Producto","id",producto.getIdProducto()));
        iProducto.save(productoBuscado);
    }
